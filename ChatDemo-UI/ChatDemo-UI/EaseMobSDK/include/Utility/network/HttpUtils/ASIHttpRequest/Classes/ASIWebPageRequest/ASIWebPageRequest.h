...
//  It is strongly recommend to set a downloadDestinationPath when using EMASIWebPageRequest
//  Also, performance will be better if your EMASIWebPageRequest has a downloadCache setup
//  Known issue: You cannot use startSychronous with an EMASIWebPageRequest

#import "ASIHTTPRequest.h"

@class EMASINetworkQueue;
...
// These correspond with the urlReplacementMode property of EMASIWebPageRequest
typedef enum _ASIURLReplacementMode {
	...
	// You must set the baseURL of a WebView / UIWebView to a file url pointing at the downloadDestinationPath of the main EMASIWebPageRequest if you want to display your content
...
@interface EMASIWebPageRequest : EMASIHTTPRequest {

	// Each EMASIWebPageRequest for an HTML or CSS file creates its own internal queue to download external resources
	EMASINetworkQueue *externalResourceQueue;
	...
	// Stores a reference to the EMASIWebPageRequest that created this request
	...
	EMASIWebPageRequest *parentRequest;

	// Controls what EMASIWebPageRequest does with external resources. See the notes above for more.
...
// If you subclass EMASIWebPageRequest, you can override this function to return different content or a url pointing at another location
...
@property (retain, nonatomic) EMASIWebPageRequest *parentRequest;
