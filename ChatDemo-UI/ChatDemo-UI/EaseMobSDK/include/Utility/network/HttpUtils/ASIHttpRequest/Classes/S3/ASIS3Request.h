...
@interface EMASIS3Request : EMASIHTTPRequest <NSCopying, NSXMLParserDelegate> {
	
	// Your S3 access key. Set it on the request, or set it globally using [EMASIS3Request setSharedAccessKey:]
	NSString *accessKey;
	
	// Your S3 secret access key. Set it on the request, or set it globally using [EMASIS3Request setSharedSecretAccessKey:]
	...
	// The access policy to use when PUTting a file (see the string constants at the top EMASIS3Request.h for details on what the possible options are)
...
// Is overridden in EMASIS3ObjectRequest
